...
import (
	...
	modelutil "github.com/tal-tech/go-zero/tools/goctl/model/pgsql/util"
...
func (g *defaultGenerator) StartFromInformationSchema(tables map[string]*model.PgTable, withCache bool) error {
	m := make(map[string]string)
	for _, each := range tables {
		table, err := parser.ConvertPgDataType(each)
		if err != nil {
			return err
		}
...
func (g *defaultGenerator) GenPgFactory(option string, tables map[string]*model.PgTable) error {
	...
	typesCode, err := genPgFactoryTypes(g.pkg, tables)
	if err != nil {
		log.Println("genTypes err", err)
		return err
	}

	funcCode, err := genPgFactoryFunc(g.pkg, tables)
	if err != nil {
		log.Println("genPgFactoryFunc err", err)
		return err
	}
